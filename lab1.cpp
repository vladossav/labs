...
void test(ArrayWork& mass) {
	...
	cout << "\nРезультат теста на добавление элемента: " << testAdd(mass);
	cout << "\n    Результат теста на замену элемента: " << testReplace(mass);
	cout << "\n  Результат теста на удаление элемента: " << testDelete(mass);
	cout << "\n Результат теста на сортировку массива: " << testSort(mass);
	cout << "\n     Результат теста на поиск элемента: " << testSearch(mass);
}

int main() {
	setlocale(LC_ALL, "Russian");
	ArrayWork mass1;
	cout << "Проверка работы конструктора по умолчанию" << endl;
	test(mass1);

	int A[5] = { -100,2,0,-54,25 };
	ArrayWork mass2(5,A);
	cout << "\n\nПроверка работы конструктора с параметрами" << endl;
	test(mass2);
